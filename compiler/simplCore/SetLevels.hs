...
data LevelEnv
  = LE { le_switches :: FloatOutSwitches
       ...
       , le_subst    :: Subst           -- ^ Domain is pre-cloned TyVars and Ids
                                        ...
                                        -- (since we want to substitute in LevelledExpr instead
...
-- | Extend the let-no-escape set
lneLvlEnv :: LevelEnv -> [Id] -> LevelEnv
lneLvlEnv env lnes = env { le_LNEs = extendDVarSetList (le_LNEs env) lnes }
    ...
    mk_poly_bndr bndr uniq = transferPolyIdInfo bndr abs_vars $         -- Note [transferPolyIdInfo] in Id.hs
                           ...
                           where
                             str     = prefix ++ occNameString (getOccName bndr)
                               where prefix | isFinalPass env  = "llf_"
                                            | otherwise        = "poly"
 ...
 ************************************************************************
*
         Determining unapplied variables}
*									*
 ************************************************************************
-}
...
************************************************************************
*                                                                      *
           Free variables (and types) and unapplied variables}
*                                                                      *
************************************************************************
-}
...
-- | An abstraction of a term retaining only information
...
costToLift :: (OldIdSet -> NewIdSet)
           -> (Id -> WordOff)
           -> NewId    -- ^ the function binder
           -> NewIdSet -- ^ the function's free variables
